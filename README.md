...
# 编译说明
- 注册登陆github账号，然后点我仓库右上角的“Fork”把我的脚本Fork到你的仓库
## 不通过SSH修改配置
- 如果有条件的可以在本地设置好需要的config文件生成，然后复制到release.config里替换内容后
# 
- 到 Actions 里面 点击 Build Base-Release-OpenWrt ，然后再点击右边的Run workflow可直接开始编译
## 通过SSH修改配置
...
- 《diy-release1.sh/diy-release2.sh》&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;文件1的编译顺序在文件2的前面，如果文件2的编译内容涉及预处理，比如需要先修改feeds.conf.default，则可把操作放在文件1里，一键编译脚本的设置自定义插件文件，想增加啥插件，基本都在这里完成了

- 《release.config》&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; 配置文件，你配置好了一次，编译完成后，在固件文件夹里面有一个config.buildinfo文件的，用记事本打开，然后把里面的内容都复制粘贴到这里，以后在不需修改任何配置的情况下，就可以把SSH连接关闭了，不需要SSH连接了，配置都是你前面设置好的，这样就可以设置定时编译了（文件名字可以随便改，保持.config格式就好了，还要在《.github/workflows》里面主文件修改好对应的名字）
